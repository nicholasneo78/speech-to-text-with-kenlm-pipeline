...
model = Wav2Vec2ForCTC.from_pretrained('./saved_model/') # cannot
processor = Wav2Vec2Processor.from_pretrained('./processor/') # cannot

# model_ = Wav2Vec2ForCTC.from_pretrained("jonatasgrosman/wav2vec2-large-xlsr-53-english") # can
# processor_ = Wav2Vec2Processor.from_pretrained("jonatasgrosman/wav2vec2-large-xlsr-53-english") # can

# model_ = Wav2Vec2ForCTC.from_pretrained("facebook/wav2vec2-large-960h") # cannot
# processor_ = Wav2Vec2Processor.from_pretrained("facebook/wav2vec2-large-960h") # cannot

# model = Wav2Vec2ForCTC.from_pretrained("facebook/wav2vec2-base-10k-voxpopuli-ft-en") # can
# processor = Wav2Vec2Processor.from_pretrained("facebook/wav2vec2-base-10k-voxpopuli-ft-en") # can

# load it locally
# model = Wav2Vec2ForCTC.from_pretrained("./test_model/") # can, xlsr53 but downloaded locally
# processor = Wav2Vec2Processor.from_pretrained("./test_model/") # can, xlsr53 but downloaded locally
...
alpha=0
beta=0
beam_width = 1 #1024 # finding all possible combinations of probability

beam_decoder = BeamCTCDecoder(vocab, lm_path=lm_path,
                                 alpha=alpha, beta=beta,
                                 cutoff_top_n=40, cutoff_prob=1.0,
                                 beam_width=beam_width, num_processes=1, #16
